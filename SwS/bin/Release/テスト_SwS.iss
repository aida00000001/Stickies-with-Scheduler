...
Source: ".\SwS.db"; DestDir: "{localappdata}"; Flags: onlyifdoesntexist; BeforeInstall: FileCopy({app}\SwS.db, {localappdata}\SwS.db); AfterInstall: DeleteFile({app}\SwS.db);
...
[Run]
Filename: "{app}\SwS.db"; Flags: shellexec skipifsilent unchecked runhidden skipifdoesntexist; WorkingDir: "{localappdata}"; BeforeInstall: FileCopy({app}\SwS.db, {localappdata}\SwS.db); AfterInstall: DeleteFile({app}\SwS.db);
